...
const FAQ = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      <h2 className="text-2xl font-bold text-center mb-6">Pixalive Gold Network – Frequently Asked Questions</h2>
...
<div className="space-y-4">
  {(faqData[activeTab] || []).map((item, index) => (
    <div key={index} className="border border-yellow-400 rounded-lg">
      <button
        onClick={() => toggleAccordion(index)}
        className="w-full text-left px-4 py-3 font-medium text-yellow-600 focus:outline-none flex justify-between items-center"
      >
      ...
      {openIndex === index && (
        <div className="px-4 py-2 text-zinc-400 whitespace-pre-line">
          {item.answer}
        </div>
      )}
    </div>
